...
public class CommandHandler implements CommandExecutor {
	...
	public CommandHandler(Main plugin) {
		...
		Optional.ofNullable(plugin.getCommand("tntrun")).ifPresent(tntrun -> {
			tntrun.setExecutor(this);
			tntrun.setTabCompleter(new TabCompletion(this));
		});
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (args.length == 0) {
			sender.sendMessage(ChatColor.DARK_AQUA + "This server is running " + ChatColor.AQUA + "TNT Run " + ChatColor.DARK_AQUA + "v" + plugin.getDescription().getVersion() + " by " + ChatColor.AQUA + "Despical");
